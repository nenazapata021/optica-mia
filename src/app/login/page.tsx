...
function LoginContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("")
    try {
      await login(form.email, form.password, callbackUrl)
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4 py-12">
      <div className="max-w-md w-full space-y-8">
        <div>
          <h1 className="text-center text-3xl font-bold text-gray-900">Iniciar Sesión</h1>
        </div>
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="text-center">
            <p className="text-sm text-gray-600">
              ¿No tienes cuenta?{" "}
              <Link href={`/registro?callback=${callbackUrl}`} className="font-medium text-indigo-600 hover:text-indigo-500">
                Regístrate
              </Link>
            </p>
