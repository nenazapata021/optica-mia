...
export default function Catalogo({
  ...
  const handleSeleccionar = (producto) => {
    // Aseguramos que el objeto producto tenga la propiedad 'imagen' correctamente
    const productoParaCarrito = {
      ...producto,
      imagen: Array.isArray(producto.imagen) ? producto.imagen[0] : producto.imagen
    };
    agregarAlCarrito(productoParaCarrito, { silencioso: true }); 
  ...
  return (
    <section className="catalogo py-12 max-w-7xl mx-auto px-4 bg-[#e2f1ee]">
      ...
      <div className="categorias flex flex-wrap justify-center gap-4 mb-8">
        <button onClick={() => handleFiltroClick("mujer")} className={getButtonClass("mujer")} style={getButtonStyle("mujer")}>Mujer</button>
        <button onClick={() => handleFiltroClick("hombre")} className={getButtonClass("hombre")} style={getButtonStyle("hombre")}>Hombre</button>
        <button onClick={() => handleFiltroClick("niños")} className={getButtonClass("niños")} style={getButtonStyle("niños")}>Niños</button>
        <button onClick={() => handleFiltroClick("sol")} className={getButtonClass("sol")} style={getButtonStyle("sol")}>Sol</button>
        <button onClick={() => handleFiltroClick("todos")} className={getButtonClass("todos")} style={getButtonStyle("todos")}>Todos</button>
      </div>

      {/* Contador de productos */}
      <h2 className="text-2xl font-bold mb-8 capitalize text-left">
        {filtro} <span className="text-gray-500 text-lg font-normal"> {" "} ({productosFiltrados.length}) </span>
      </h2>
      ...
      <div className="productos grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {productosFiltrados.map((producto) => (
          <div
            key={producto.id}
            className="group border rounded-lg shadow-sm p-4 flex flex-col bg-white overflow-hidden"
          >
            <Link href={`/lentes/${producto.id}`} className="flex flex-col [flex-grow]">
              ...
              <div className="mt-auto pt-4 border-t border-gray-100">
                ...
                <button
                  ...
                  className="w-full mt-2 text-slate-900 py-2 rounded-md transition text-sm font-medium hover:opacity-90"
